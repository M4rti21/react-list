...
export type LocalSorter<T> = {
    name: string;
    desc?: string;
    action: (a: T, b: T) => number;
};

export type LocalSearcher<T> = {
    name: string;
    desc?: string;
    value: string;
...
type Props<T> = {
    items: T[];
    filters?: LocalFilter<T>[];
    sorters?: LocalSorter<T>[];
    searchers?: LocalSearcher<T>[];
...
function LocalList<T>({
    ...
    const [activeFilters, setActiveFilters] = useState<LocalFilter<T>[]>([]);
    const [activeSorter, setActiveSorter] =
        useState<LocalSorter<T>>(defaultSorter);
